...
// Shared code/docs signal (also used by test-runnability + ci-config) so a
// manifest-less *code* repo isn't mislabeled "docs-only". MANIFESTS is consumed
// by code-presence lazily (inside repoHasCode's body), so this import cycle is
// resolved by call time — no eval-order hazard.
import { repoHasCode } from './code-presence.js';
...
export const setupReproducibility = {
  ...
  run(ctx) {
    ...
    if (!hasManifest) {
      ...
      if (!envPin) {
        // Don't guess "docs-only" when the repo demonstrably has code (source
        // files, or per-package manifests in subdirs) but simply keeps no
        // dependency manifest at its *root* — e.g. a Makefile-driven project, or
        // a monorepo whose manifests live under subdirectories. Calling that a
        // docs repo contradicts the test-runnability/ci-config checks (which
        // detect its code via the same repoHasCode signal) and self-contradicts
        // in the report. Same neutral 0.7 either way — with no manifest we can't
        // assess dependency pinning — but an honest message.
        return repoHasCode(ctx)
          ? { score: 0.7, details: 'no dependency manifest at the repo root (a Makefile-driven project, or per-package manifests in subdirectories)', fix: 'If a fresh agent must install runtime dependencies to work here, declare them in a root manifest with a lockfile (or pin the environment via a devcontainer / .tool-versions) so setup is reproducible.' }
          : { score: 0.7, details: 'no dependency manifest found (docs-only repo?)', fix: 'If this repo has runtime dependencies, declare them in a manifest with a lockfile.' };
      }
