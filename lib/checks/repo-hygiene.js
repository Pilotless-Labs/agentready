...
// TypeScript is a compile-FROM language: toolchains emit .js and .d.ts, never
// plain .ts — so non-declaration TS inside an artifact-named dir is authored
// source (e.g. hono commits its build scripts + their tests as `build/*.ts`).
// Declaration files (.d.ts/.d.mts/.d.cts) ARE emitted output and never exempt,
// which keeps a typical dist/ of .js + .d.ts flagged.
const TS_SOURCE = /\.(ts|tsx|mts|cts)$/i;
const TS_DECLARATION = /\.d\.(ts|mts|cts)$/i;
...
function isSourceDir(ctx, dir) {
  ...
  return contents.some((f) => {
    ...
    return EXEMPT_SOURCE.test(base) || (TS_SOURCE.test(base) && !TS_DECLARATION.test(base));
  });
}
