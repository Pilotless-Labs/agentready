...
import { isDocsOnlyRepo } from './code-presence.js';
...
// A basename can match BOTH families (e.g. `doc-check-links` matches PRIMARY via
// the `checks?` token AND SECONDARY via `docs?`): such a workflow is really the
// secondary one (a scheduled docs link-checker), so a config that is primary
// *and not also secondary* must outrank one that merely matches primary. Verified
// on pytest-dev/pytest, whose `doc-check-links.yml` (`tox -e docs-checklinks`,
// sorts first) was shadowing the genuine `test.yml` (`tox` test envs, `name: test`).
function pickCiConfig(matches) {
  const base = (f) => f.split('/').pop().replace(/\.ya?ml$/, '');
  const isPrimary = (f) => PRIMARY_CI_NAME.test(base(f));
  const isSecondary = (f) => SECONDARY_CI_NAME.test(base(f));
  return matches.find((f) => isPrimary(f) && !isSecondary(f))
    ?? matches.find((f) => isPrimary(f))
    ?? matches.find((f) => !isSecondary(f))
    ?? matches[0];
}
...
export const ciConfig = {
  ...
  run(ctx) {
    ...
    if (configs.length === 0) {
      // No CI on a docs/content-only repo (no manifest, no source files) is the
      // expected shape, not a defect — mirror the test-runnability /
      // setup-reproducibility softening instead of hard-failing a repo that has
      // nothing to build or test.
      if (isDocsOnlyRepo(ctx)) {
        return {
          score: 0.7,
          details: 'no CI configuration (docs/content-only repo?)',
          fix: 'If this repo has code, add a CI workflow that runs the test suite on every push/PR (e.g. .github/workflows/test.yml).',
        };
      }
      return {
        score: 0,
