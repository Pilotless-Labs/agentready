...
export const testRunnability = {
  ...
  run(ctx) {
    ...
    if (!hasTestFiles && !command) {
      // No test suite AND no code to test (no manifest, no source files) =>
      // a docs/content/template repo, not a project missing tests. Hard-failing
      // it mis-advises ("add a test suite" when there is nothing to test); score
      // it neutral, the way setup-reproducibility softens a missing manifest.
      if (isDocsOnlyRepo(ctx)) {
        return {
          score: 0.7,
          details: 'no code to test (docs/content-only repo?)',
          fix: 'If this repo has code, add a test suite wired to a standard entry point (npm test, pytest, cargo test...) so agents can verify changes.',
        };
      }
      return {
        score: 0,
