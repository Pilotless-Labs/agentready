...
const INSTRUCTION_FILES = ['CLAUDE.md', 'AGENTS.md', '.github/copilot-instructions.md'];
...
export const agentInstructions = {
  ...
  weight: 25,
  run(ctx) {
    const found = INSTRUCTION_FILES.filter((f) => ctx.exists(f));
    if (found.length === 0) {
      return {
        score: 0,
        details: 'no CLAUDE.md, AGENTS.md, or .github/copilot-instructions.md',
    ...
    if (wordCount < 30) {
      return {
        score: 0.3,
        details: `${found.join(', ')} exists but is nearly empty (${wordCount} words${ph.markers ? ` once ${ph.markers} placeholder line(s) are ignored` : ''})`,
    ...
    return {
      score,
      details: `${found.join(', ')} covers ${covered.length}/${TOPICS.length} core topics${ph.markers ? `, ${ph.markers} unfilled placeholder line(s)` : ''}`,
