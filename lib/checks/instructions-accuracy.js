...
export const instructionsAccuracy = {
  ...
  run(ctx) {
    ...
    if (refs.length === 0) {
      return { score: 1, details: `${found.join(', ')}: no verifiable path/command references` };
    }

    const broken = refs.filter((ref) => !verify(ctx, ref));
    if (broken.length === 0) {
      return { score: 1, details: `all ${refs.length} path/command references in ${found.join(', ')} resolve` };
    }

    const shown = broken.slice(0, 3).map((r) => `\`${r.span}\``).join(', ');
    return {
      score: (refs.length - broken.length) / refs.length,
      details: `stale references in ${found.join(', ')}: ${shown}${broken.length > 3 ? ` (+${broken.length - 3} more)` : ''}`,
